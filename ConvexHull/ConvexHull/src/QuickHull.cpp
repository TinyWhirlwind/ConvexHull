...
QuickHull::QuickHull(const CMeshO& mesh)
{
	CMeshO vert_mesh;
	CMeshO::VertexIterator vIter = vcg::tri::Allocator<CMeshO>::AddVertices(vert_mesh, mesh.vert.size());
	for (int j = 0; j < mesh.vert.size(); j++)
	{
		(*vIter).P().Import(mesh.vert[j].P());
		(*vIter).N().Import(mesh.vert[j].N()); // 复制法线
		vIter++;
	}
	impl_.reset(new PImpl(vert_mesh));
}

QuickHull::QuickHull(const std::vector<CMeshO>& mesh)
{
	CMeshO vert_mesh;
